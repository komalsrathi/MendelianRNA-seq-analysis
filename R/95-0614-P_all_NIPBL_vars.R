...
# Date: 03/19/2019
# Function: script to further investigate 95-0614-P

library(data.table)
setwd('~/Projects/DGD_Mendelian_RNASeq/')
dat <- fread('data/variant_filtering/rawdata/vardict/Sample_1__95-0614-P.vardict-annotated-rnaedit-annotated-gemini-hgmdannotated.maf')
...
dat <- fread('data/variant_filtering/rawdata/gatk4/95-0614-P-gatk-haplotype-annotated-hgmdannotated.maf')
dat <- dat[which(dat$Hugo_Symbol == "NIPBL"),]
# gatk <- dat[grep('Splice|Intron', dat$Variant_Classification, ignore.case = T),c('Start_Position','End_Position','HGVSc','variant_qual','Variant_Classification','EXON','INTRON','RefSeq')]
gatk <- dat[,c('Start_Position','End_Position','HGVSc','variant_qual','Variant_Classification','EXON','INTRON','RefSeq')]
gatk$caller <- 'GATK'

dat <- fread('data/variant_filtering/rawdata/strelka/95-0614-P.variants-hgmdannotated.maf')
dat <- dat[which(dat$Hugo_Symbol == "NIPBL"),]
# strelka <- dat[grep('Splice|Intron', dat$Variant_Classification, ignore.case = T),c('Start_Position','End_Position','HGVSc','variant_qual','Variant_Classification','EXON','INTRON','RefSeq')]
...
total$splice_coords <- ifelse((total$Start_Position >= 37010327 & total$End_Position <= 37012278) | (total$Start_Position >= 37012394 & total$End_Position <= 37014785), 'Within_splice_coords','Outside')
total <- total[which(total$splice_coords == "Within_splice_coords"),]
write.table(total, file = 'data/95-0614-P-splice-and-intronic-variants-in-NIPBL.txt', quote = F, sep = "\t", row.names = F)
...
x <- read.delim('data/95-0614-P-splice-and-intronic-variants-in-NIPBL.txt', stringsAsFactors = F)
x <- unique(x$HGVSc)
