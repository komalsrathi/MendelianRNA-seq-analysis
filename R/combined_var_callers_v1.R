...
# Date: 04/30/2019
# Function: Combined Variant caller plot 
...
gatk$caller <- 'GATK4'
total <- rbind(gatk, vardict, strelka)
...
total$caller <- factor(total$caller, levels = c('Strelka','GATK4','Vardict'))
...
p <- ggplot(total, aes(x = variable, y = value, group = caller)) + 
  geom_bar(position = position_dodge(width=0.7), stat="identity", 
           width=0.7, aes(fill = variable), color = 'black', size = 0.2) +
...
p
ggsave(filename = 'paper/variant_plots/combined_variant_plot_v1.pdf', plot = p, width = 8, height = 6)
